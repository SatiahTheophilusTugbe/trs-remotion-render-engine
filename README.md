...
- SSRF rules (`assertSafeUrl`): only `https:` is fetched; `localhost`, `*.localhost`, `*.internal`, `*.local` and literal IPs in loopback, private (10/8, 172.16/12, 192.168/16), link-local/metadata (169.254/16), unspecified (0/8), CGNAT (100.64/10), IPv6 `::1`, `::`, `fc00::/7`, `fe80::/10` and IPv4-mapped/NAT64 forms are refused. Hostnames are DNS-resolved before every hop and refused if ANY address is in a blocked range. Redirects are followed manually (max 3 hops) and every `Location` (relative ones resolved) is re-checked (https only + the same rules). Blocked targets return the same 422 with `status=blocked` (or `too_many_redirects`). Known limitation: fetch re-resolves DNS itself, so a DNS-rebinding race remains possible in a tiny window.
- Size cap: 15 MB is enforced while streaming (the read is cancelled as soon as it is exceeded); a missing or lying `Content-Length` cannot bypass it, and a declared oversize length is rejected before reading.
- Time budget: 10 s per photo and a 45 s global deadline for the whole re-hosting step; photos not started by then fail fast with `status=timeout` (HTTP 422). `vercel.json` sets `maxDuration: 60` for `api/submit-render.ts`.
- Missing `REMOTION_AWS_ACCESS_KEY_ID`, `REMOTION_AWS_SECRET_ACCESS_KEY`, `REMOTION_REGION`, `REMOTION_FUNCTION_NAME` or `REMOTION_SERVE_URL` returns HTTP 500 `{ "error": "server misconfigured" }` (no values) after the auth check.
- Re-hosted objects are not deleted automatically. HUMAN follow-up: add an S3 lifecycle rule expiring objects under the `assets/` prefix (e.g. after 7 days) on the bucket.
